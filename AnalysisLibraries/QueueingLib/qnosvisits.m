...
function V = qnosvisits( P, lambda )
  ...
  assert( nargin == 2 ,'Input signature is 2 arguments: P and lambda')
  
  assert(issquare(P) == 1, 'P must be a square matrix');
  
  K = size(P,1);

  assert(all(P(:)>=0) && all( sum(P,2)<=1+1e-5),'invalid transition probability matrix P');
  
  assert(isvector(lambda) && length(lambda) == K, 'lambda must be a vector with %d elements', K);
