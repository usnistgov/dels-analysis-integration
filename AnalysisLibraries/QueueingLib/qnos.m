...
function [U, R, Q, X] = qnos( varargin )
  ...
  assert(isempty(err)==1, err);

  assert(all(S>0), 'S must be positive');
  ...
  %NOTE 4/12/16 There needs to be a filter for stations with 0 arrival rate
  %since the multiple machines requires l >0
  %This can arise in randomly generated instances.
  ii = find(l == 0);
  if numel(ii)
      U(ii) =0; R(ii) = 0; Q(ii) = 0; X(ii) = 0;
      m(ii) = NaN;
  end

  ii = find( m == 1 ); % single station queueing centers
  if numel(ii)
    [U(ii), R(ii), Q(ii), X(ii)] = qsmm1( l(ii), 1./S(ii) );
  end
  
  ii = find( m<1 ); % delay centers
  if numel(ii)
    [U(ii), R(ii), Q(ii), X(ii)] = qsmminf( l(ii), 1./S(ii) );
  end
  
  ii = find( m>1 ); % multiple stations queueing centers
  if numel(ii)
    [U(ii), R(ii), Q(ii), X(ii)] = qsmmm( l(ii), 1./S(ii), m(ii) );
  end
end
%!test
%! lambda = 0;
