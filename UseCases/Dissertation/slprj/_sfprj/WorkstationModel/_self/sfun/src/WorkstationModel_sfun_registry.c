...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_WorkstationModel_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_WorkstationModel_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_WorkstationModel_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "WorkstationModel") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_WorkstationModel_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_WorkstationModel_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "WorkstationModel") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_WorkstationModel_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_WorkstationModel_method_dispatcher(SimStruct *S, unsigned
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"WorkstationModel")) {
    return(sf_WorkstationModel_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void WorkstationModel_terminator(void);
...
extern void WorkstationModel_initializer(void);
extern void WorkstationModel_register_exported_symbols(SimStruct* S);
extern void WorkstationModel_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  WorkstationModel_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      WorkstationModel_debug_initialize();
    }

    WorkstationModel_initializer();
  }
