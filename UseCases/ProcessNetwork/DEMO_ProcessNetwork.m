...
nProd = 15;
nProcess = 50;
lengthProcessPlan = 20; %all process plans are the same length for now
...
%Set Number of machines at each workstation
    concurrentProcessingCapacity = ceil(sum(externalArrivalRate) * serviceTime .* avgNoVisits / 0.95);
    for ii = 1:length(processSet)
...
PN = ProcessNetwork;
    ...
    processPlanList = zeros(length(productSet),20);
    ...
    %Build routing probability
        ...
        PN.processNodeSet{1}(end+1) = arrivalProcess;


        %3) aggregate all departures from the process network into one (new) departure process node (a sink node)
        [departureProcess, departureFlowEdgeSet] = mapDepartures2DepartureProcessNode(PN.processNodeSet{1}, PN.flowEdgeSet, PN.probabilityTransitionMatrix);
        %Add departureProcessNode to ProcessNode Set;
        PN.flowEdgeSet(end+1:end+length(departureFlowEdgeSet))= departureFlowEdgeSet;
        PN.processNodeSet{1}(end+1) = departureProcess;

        %4) With all new process steps and flow edges, create necessary connectors/references 
        
        for ii = 1:length(PN.processNodeSet{1})
            processNode = PN.processNodeSet{1}(ii);
            processNode.addEdge(flowEdgeSet); 

            for jj = 1:length(processNode.inFlowEdgeSet)
                processNode.inFlowEdgeSet(jj).targetFlowNetwork = processNode;
            end

            for jj = 1:length(processNode.outFlowEdgeSet)
                processNode.outFlowEdgeSet(jj).sourceFlowNetwork = processNode;
            end
...
simFac.inputFlowNetwork = PN;
    ...
    for ii = 1:length(PN.processNodeSet)
        for jj = 1:length(PN.processNodeSet{ii})
            ...
            builderSet(jj).analysisTypeID = PN.processNodeSet{ii}(jj).typeID;
            ...
            builderSet(jj).setSystemElement(PN.processNodeSet{ii}(jj));
        end
    end
